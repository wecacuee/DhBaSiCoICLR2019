...
% TODO : The flow is not there, what are the limitations. why do we do what we do?
    ...
    associating them with high reward.
    %it also introduces additional goal reward sampling steps in algorithms like Hindsight
    %Experience Replay (HER). HER uses episodes in which the agent
    %fails to reach the goal by resampling rewards as if the the reached states
    %were pseudo-desired goals.
    This dependence introduces additional goal reward resampling steps 
    in algorithms like Hindsight Experience Replay which reuse
    episodes in which the agent fails to reach the goal by recomputing
    rewards as if reached states were psuedo-desired goals. 
    We propose a reformulation of goal-conditioned value functions for
    MGRLs that yield a similar algorithm, while
    ...
    neural networks. Our results are competetive with HER while
