...
Two types of Reinforcement Learning (RL) algorithms are used for multi-goal tasks, 
\emph{model-based} or \emph{model-free} depending on whether state-dynamics are
learned. Each of these approaches has its limitations.
Model-free (RL) struggles to transfer learned information when the goal location
changes, but achieves high asymptotic performance. Model-based
...
Our project is to address multi-goal tasks by improving upon the limitations of
model-free RL. We do this by adapting the Floyd-Warshall algorithm to
the RL domain. We call our adaptation Floyd-Warshall RL (FWRL).

Our algorithm
works by learning a goal-conditioned value function. We constrain the value of
optimal path between any two states to be greater than or equal to the value of
path via any intermediate state. FWRL is experimentally shown to transfer
knowledge about an environment when the goal location is dynamic compared to a
model-based Q-learning baseline.
\end{abstract}
