...
%
In this paper, we demonstrate that \emph{goal-rewards are not needed to learn
GCVFs}. For the Fetch-Push task, we demonstrate that ``0'' goal-reward is not
needed to learn the GCVFs. The GVCFs are learned even when the ``0'' goal-reward
is replaced by the ``-1'' goal-independent reward. This reward formulation is
atypical in conventional RL. Conventional RL algorithms maximize cumulative
reward. Hence a high reward is used to specify the desired goal---the
goal-reward. However, this goal-reward is not necessary in goal-conditioned RL
because the goal is already specified at the start of every episode.
We use this idea, to propose a goal-conditioned RL algorithm which learns to
reach goals without goal-rewards. We believe that this counter-intuitive
result is important for understanding goal-conditioned RL.
